...
public class DesktopParserTest extends ParserTest{
	...
	public void shouldParseCurrency() throws Exception {
		...
		Desktop desktop = (Desktop) parser.parse(parseHtml(product), product, config);
		assertEquals("USD", desktop.getProduct().getPrices().get(HP).getCurrency());
	}
	...
	public void shouldParseAndPopulateCurrentPrice() throws Exception {
		...
		assertEquals(new BigDecimal("399.99"), desktop.getProduct().getPrices().get(HP).getCurrentPrice());
	...
	public void shouldParseAndPopulateStrikedPrice() throws Exception {
		...
		assertEquals(new BigDecimal("3147.00"), desktop.getProduct().getPrices().get(HP).getStrikedPrice());
	}
	...
	public void shouldParseAndPopulateRatingAndReviewsCount() throws Exception {
		...
		Desktop desktop = (Desktop) parser.parse(parseHtml(product), product, config);
		assertEquals(Integer.valueOf(4), desktop.getProduct().getRatings().get(HP).getRating());
		assertEquals(Integer.valueOf(20), desktop.getProduct().getRatings().get(HP).getNumberOfReviews());
	}
	...
	public void shouldUpdatePreviousRating() throws Exception {
		...
		Desktop desktopOriginal = (Desktop) parser.parse(parseHtml(productOriginal), productOriginal, config);
		Desktop desktopUpdated = (Desktop) parser.parse(parseHtml(productUpdated), productUpdated, config);
	...
	@Test
	public void shouldParseSpeakers() throws Exception {
		Product product = findProduct("productPage1384.html");
		Desktop desktop = (Desktop) parser.parse(parseHtml(product), product, config);
